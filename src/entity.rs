...
impl ComponentType {
    fn of<T: 'static>() -> Self {
        fn dtor<T: 'static>(entity: Entity) {
            drop(entity.remove::<T>()); // (ignores missing components)
        }
...
impl Entity {
    ...
    pub fn insert<T: 'static>(self, comp: T) -> Option<T> {
        ALIVE.with(|slots| {
            ...
            let slot = slots.get_mut(&self).unwrap_or_else(|| {
                panic!("attempted to attach a component to the dead {:?}.", self)
            });
    ...
    pub fn remove<T: 'static>(self) -> Option<T> {
        ALIVE.with(|slots| {
            let mut slots = slots.borrow_mut();
            let slot = slots.get_mut(&self).unwrap_or_else(|| {
                panic!("attempted to remove a component from the dead {:?}.", self)
            });
